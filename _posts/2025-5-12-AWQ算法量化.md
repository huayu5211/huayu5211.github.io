...
量化是将浮点数（如 FP32，32位浮点数）映射到低比特整数（如 INT8，8位整数）表示的过程。核心目标是减少存储需求和计算量，同时控制精度损失。量化的数学原理基于线性映射：
#### 公式：
对于一个浮点数 ( x )，量化到整数 ( q ) 的过程为：
q=round(x−z)/ s)

#### 其中：
( s )：缩放因子（scale），表示浮点数范围与整数范围的映射比例。

( z )：零点（zero-point），用于处理非对称量化。

( q )：量化后的整数值，范围通常是 [-2^{b-1}, 2^{b-1}-1]（对于 ( b )-bit 整数）。

#### 反量化则是：
x′=s⋅q+z

其中 ( x' ) 是近似的浮点值，可能与原始 ( x ) 有量化误差。
#### 对称与非对称量化：
对称量化：假设浮点数分布对称，零点 z=0，只需一个缩放因子 ( s )。

非对称量化：引入零点 ( z ) 来处理非对称分布（如 ReLU 激活后的值）。

#### 其它：

校准数据：用于模拟真实输入，捕获激活值分布

过程：每个线性模块的权重计算最优缩放因子，减少量化误差,在“通道(channel)”级别进行.即权重矩阵的一行作为一个单位,确定对应的量化因子。
